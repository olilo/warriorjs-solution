class Player {
  playTurn(warrior) {
    ...
    if (this._nextSpaces[0].isEmpty() && this._nextSpaces[1].isEmpty() && this._nextSpaces[2].isEnemy()) {
      warrior.shoot('backward');
      return;
    }
    ...
    if (warrior.feel().isWall()) {
      warrior.pivot();
    } else if (warrior.feel('backward').isEmpty() && (/*(warrior.health() < 8 && this._health > warrior.health()) || */!this.walkForward)) {
      warrior.walk('backward');
    } else if (warrior.feel('backward').isCaptive() && !this.walkForward) {
